...
static bool nr_ue_fuzz_hook_apply_txn_value(NR_UE_RRC_INST_t *rrc, long *value, const char *mode)
{
  ...
  if (mutation->transform_name[0] && !echoed
      && !nr_ue_fuzz_hook_text_eq(mutation->transform_name, "domain_value_selection"))
    return false;
  ...
  for (size_t c = 0; c < sizeof(catalogs) / sizeof(catalogs[0]); ++c) {
    for (size_t i = 0; i < catalogs[c].count; ++i) {
      ...
      if (!nr_ue_fuzz_hook_text_eq(mutation->message, adapter->message_name)
        ...
        continue;
      if (found) {
        LOG_W(NR_RRC, "[UE HOOK] duplicate adapter identity for %s.%s:%s\n",
              mutation->message, mutation->field, mutation->operator_family);
        return NULL;
      }
      found = adapter;
    }
...
/* One pre-encode callback for both field and transaction adapters. */
static bool nr_ue_fuzz_hook_mutate_ul_dcch(void *context, NR_UL_DCCH_Message_t *pdu)
{
  ...
  if (!payload || msg != hook->target_msg)
    return false;
